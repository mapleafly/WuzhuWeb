...
依赖：docker compose v2，删除`name: wuzhu-system`，降低`version`版本为`3.3`以下，支持`docker-compose`。
...
```shell
# 创建maven缓存volume
docker volume create --name wuzhu-maven-repo

docker run -it --rm --name wuzhu-maven \
    -v wuzhu-maven-repo:/root/.m2 \
